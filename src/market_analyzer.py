...
"""
    ...
    def _build_review_prompt(self, overview: MarketOverview, news: List) -> str:
        ...
        # 按 region 组装市场概况与板块区块（美股无涨跌家数、板块排行数据）
        stats_block = ""
        sector_block = ""
        if review_language == "en":
            if self.profile.has_market_stats:
...
- 两市成交额: {overview.total_amount:.0f} 亿元"""
                ...
                stats_block = (
                    "## 市场概况\n"
                    "（该市场的涨跌家数统计暂未接入，不是数据错误；请主要根据指数涨跌和新闻线索判断今天市场冷暖。）"
                )
...
概念领跌: {bottom_concepts_text if bottom_concepts_text else "暂无数据"}"""
                ...
                sector_block = (
                    "## 板块表现\n"
                    "（美股板块排行暂未接入，不是数据错误；请根据指数强弱、新闻线索和常见主题推断主线，并明确哪些判断是推断。）"
                )
...
### 7. Strategy Plan
(Provide a plain stance, a simple position-sizing guideline, one invalidation trigger, and end with “For reference only, not investment advice.”)
...
"""
        ...
        return f"""你是一位会把股市信息讲清楚的每日简报助手，请根据以下数据生成一份通俗易懂的{self._get_market_scope_name('zh')}大盘复盘。
...
- 报告要像每日简报：先说结论，再说原因、风险、明天重点看什么
- 少用专业术语；必须使用时，用一句话解释成普通人能懂的意思
- 每段尽量短，优先回答“今天发生了什么、为什么、我明天该看哪里”
- 不要重复列出已由系统注入的表格数据；正文负责解释表格背后的含义
- 数据暂未接入时，不要写得像报错；要说明这是系统暂未接入的正常限制，并换用可用线索分析
...
### 一、盘面总览
（2-3句话先说结论：今天市场偏强、偏弱还是分化；谁带动；普通人该如何理解）

### 二、指数结构
（{self._get_index_hint()}；用通俗语言说明哪个指数更强、哪个拖后腿，以及关键观察位置）

### 三、板块主线
（说明今天大家主要在买什么方向、为什么买；若没有板块排行，就根据指数和新闻推断，并明确这是推断）

### 四、资金与情绪
（用“钱更愿意进攻还是观望”的方式解释；若市场宽度或成交额暂未接入，要自然说明，不要当成利空）

### 五、消息催化
（把近三日新闻翻译成普通话：哪条消息可能推动市场，哪条可能带来扰动）

### 六、明日交易计划
（直接写明天重点看什么、哪些方向可关注、哪些情况要小心，以及一个看错的信号）

### 七、风险提示
（列出最容易让判断失效的风险；最后补充“建议仅供参考，不构成投资建议”。）
...
"""
        ...
        if not dashboard_block:
            dashboard_block = (
                f"- {market_label}涨跌家数统计暂未接入，不是数据错误；"
                "下面根据指数表现和新闻线索判断市场冷暖。"
            )
        if not sector_block:
            if self.region == "us":
                sector_block = (
                    "- 美股板块排行暂未接入，不是数据错误；"
                    "主线判断会根据纳指、标普、道指强弱和新闻线索做推断。"
                )
            else:
                sector_block = (
                    f"- {market_label}板块排行暂未接入，不是数据错误；"
                    "主线判断会根据指数和新闻线索做推断。"
                )
        return f"""## {overview.date} 大盘复盘

> 今日{market_label}市场整体呈现**{market_mood}**，先看主要指数是否还能稳住，再看热点是否继续扩散。
...
### 四、资金与情绪
- 如果涨势主要集中在少数大盘股，说明市场还不算全面转强；如果更多股票跟涨，情绪会更健康。

### 五、消息催化
- 有明确新闻推动的方向更容易延续；如果只是单日上涨，明天要看是否还能放量跟进。
